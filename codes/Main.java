	...
	public static void main(String[] args) {
						...
						try {
							...
							if (!flag && i<folder.size()){
						     	for (;i<folder.size();i++){
								    if (folder.get(i).lastModified()!=folder_last_modified.get(i)){
								    	System.out.println(folder.get(i).lastModified()+" "+folder_last_modified.get(i));
									    flag=true;
								       }
						      	   }
							   }
							...
							if (flag){
								...
								for (;i<combs.size();i++){
									...
									data_base.addAll(combiningData.list(combs.get(i).getName()));
								}
	   ...
	   /* Thread b=new Thread(new Runnable() {
		  public void run() {
				...
				try {
					frame1 window = new frame1(data_base,data_not_filtered,folder,folder_last_modified,combs,combs_last_modified);
					window.frame.setVisible(true);
